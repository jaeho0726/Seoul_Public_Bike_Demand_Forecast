...
def get_daily_data(date, station_data):
    entire_rows = []
    ...
    ### Check whether every station was successfully mapped
    missing_station_count = (
        data_df_merged["STA_LOC"]
        .isna()
        .sum()
    )

    if missing_station_count > 0:
        print(
            f"{date}: "
            f"{missing_station_count} rows have "
            f"missing district information."
    )
    ...
    if len(data_df_merged) != 25:
        print(f"{date}: expected 25 districts, "
            f"but got {len(data_df_merged)}"
        )
...
for date in dates: 
    ...
    try:
        daily_df = get_daily_data(date_str, station_data)
